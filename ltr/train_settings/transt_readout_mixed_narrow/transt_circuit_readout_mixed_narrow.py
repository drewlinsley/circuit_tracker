...
import ltr.models.tracking.transt_circuit_readout_mixed_narrow as transt_models
...
def run(settings):
    ...
    dataset_train = sampler.TransTSampler([got10k_train, trackingnet_train], [1, 1], samples_per_epoch=1000*settings.batch_size, max_gap=100, processing=data_processing_train, num_search_frames=settings.sequence_length, frame_sample_mode="causal")
    ...
    param_dicts = [
        ...
        # },
        {
            "params": [p for n, p in model.named_parameters() if "circuit" in n or "mix" in n or "new" in n or "rnn" in n],  #  or "class_embed" in n or "bbox_embed" in n],
            "lr": 1e-3,
        },
        # {
