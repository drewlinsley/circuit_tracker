...
def run(settings):
    ...
    settings.sequence_length = 10  # 30  # 64 NEXT  # Same as PT
    settings.rand = True
    ...
    dataset_train = sampler.TransTSampler([got10k_train], [1], samples_per_epoch=2*settings.batch_size, max_gap=100, processing=data_processing_train, num_search_frames=settings.sequence_length, frame_sample_mode="interval")
    ...
    actor = actors.BaselineTranstActor(net=model, objective=objective)
