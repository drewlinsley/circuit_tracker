...
import ltr.models.tracking.transt_circuit_readout_test_v1 as transt_models
...
def run(settings):
    ...
    settings.sequence_length = 15  # 30  # 64 NEXT  # Same as PT
    settings.rand = True
    ...
    dataset_train = sampler.TransTSampler([lasot_train, got10k_train, trackingnet_train], [1, 1, 1], samples_per_epoch=1000*settings.batch_size, max_gap=settings.sequence_length * 3, processing=data_processing_train, num_search_frames=settings.sequence_length, frame_sample_mode="rnn_interval")
    ...
    param_dicts = [
        ...
        # },
        {
            "params": [p for n, p in model.named_parameters() if "encoder" not in n or "input_proj" not in n or "decoder" not in n or "backbone" not in n or "embed" not in n],  # "exc" in n or "circuit" in n or "mix" in n or "new" in n or "rnn" in n or "step" in n],  #  or "class_embed" in n or "bbox_embed" in n],
            "lr": 1e-4,
        },
        # {
    ...
    print("Higher lr:")
    print([n for n, p in model.named_parameters() if "encoder" not in n and "input_proj" not in n and "decoder" not in n and "backbone" not in n and "embed" not in n])
    print("*" * 60)
    ...
    optimizer = torch.optim.AdamW(param_dicts, lr=1e-4,  # lr=1e-4,
                                  weight_decay=1e-4)
