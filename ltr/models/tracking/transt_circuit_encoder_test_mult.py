...
class TransT(nn.Module):
    ...
    def track(self, search, bumps, info):
        ...
        if 0:
            from matplotlib import pyplot as plt
            plt.subplot(141);plt.imshow(ims.squeeze().permute(1, 2, 0).cpu());plt.subplot(142);plt.imshow((self.input_proj(src_search).squeeze() ** 2).mean(0).detach().cpu());plt.subplot(143);plt.imshow((prev_hs.sigmoid()).squeeze().mean(0).detach().cpu())
            plt.subplot(144);plt.imshow(((self.input_proj(src_search)) * prev_hs.sigmoid() ** 2).squeeze().mean(0).detach().cpu());plt.show()
            plt.show()
