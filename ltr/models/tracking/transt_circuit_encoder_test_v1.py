...
from ltr.models.neck.circuit_featurefusion_encoder_test import build_featurefusion_network
...
class TransT(nn.Module):
    ...
    def __init__(self, backbone, featurefusion_network, num_classes, input_dim=32, rnn_dims=64, timesteps=32):
        ...
        self.exc_bbox_1 = nn.Conv3d(256, 1, kernel_size=[1, 3, 3], padding=[1//2, 3//2, 3//2])
        # self.exc_bbox_2 = nn.Conv3d(self.rnn_dims // 2, 1, kernel_size=[1, 3, 3], padding=[1//2, 3//2, 3//2])
        self.height = 32
        ...
        self.step3_trans = nn.Conv2d(self.rnn_dims, self.rnn_dims, kernel_size=1)  # Init states
        self.step2_trans = nn.Conv2d(self.rnn_dims, self.rnn_dims, kernel_size=1)

        self.rnn_decode_1 = nn.Conv3d(self.rnn_dims, hidden_dim, kernel_size=1)
        self.rnn_decode_2 = nn.Conv3d(hidden_dim, hidden_dim, kernel_size=1)
        torch.nn.init.constant_(self.rnn_decode_2.bias, 0)
        ...
        self.circuit_td = ConvGRUCell(input_dim=self.rnn_dims, hidden_dim=self.rnn_dims, kernel_size=3, padding_mode='zeros', batchnorm=True, use_attention=True, timesteps=self.timesteps)
        self.circuit_2 = ConvGRUCell(input_dim=self.rnn_dims, hidden_dim=self.rnn_dims, kernel_size=3, padding_mode='zeros', batchnorm=True, use_attention=True, timesteps=self.timesteps)
        self.circuit_1 = ConvGRUCell(input_dim=backbone.num_channels, hidden_dim=self.rnn_dims, kernel_size=3, padding_mode='zeros', batchnorm=True, use_attention=True, timesteps=self.timesteps)
        # self.inh_1_proj = nn.Conv2d(backbone.num_channels, backbone.num_channels, kernel_size=1)
        self.bn = nn.GroupNorm(8, self.rnn_dims, affine=False)  # nn.BatchNorm2d(self.rnn_dims, eps=1e-03, affine=False, track_running_stats=True)
        self.exc_1_init = nn.Conv2d(backbone.num_channels, self.rnn_dims, kernel_size=1)
        self.inh_1_init = nn.Conv2d(backbone.num_channels, backbone.num_channels, kernel_size=1)
        # self.td_inh_proj = nn.Conv2d(self.rnn_dims, self.rnn_dims, kernel_size=1)
    ...
    def forward(self, search, template, samp_idx, labels, settings, boxes):
        ...
        for t in range(src_search.shape[1]):
            ...
            # Step 1, saliency
            pre_exc_1, inh_1 = self.circuit_1(self.nl(src_search[:, t]), excitation=exc_1, inhibition=inh_1)

            # Step 2, tracking
            exc_2, inh_2 = self.circuit_2(self.step2_trans(pre_exc_1), excitation=exc_2, inhibition=inh_2, label=proc_labels)
        ...
        proc_rnn = self.rnn_decode_2(self.nl(dec_rnn))  # * torch.sigmoid(self.rnn_gate(dec_rnn))
        ...
        proc_rnn = self.exc_bbox_3(self.exc_bbox_1(proc_rnn).view(proc_rnn.shape[0], proc_rnn.shape[2], -1)).sigmoid()
        if self.vj_pen:
    ...
    def track(self, search, bumps, info):
        ...
        if self.reset_hidden:  # not hasattr(self, "exc"):
            exc_1, inh_1 = self.nl(self.exc_1_init(src_search)), self.nl(torch.zeros_like(src_search))
            exc_2, inh_2 = None, None
        else:
        ...
        # Step 0 init
        bumps = F.interpolate(bumps, src_search.shape[-2:])

        # Step 1, saliency
        pre_exc_1, inh_1 = self.circuit_1(src_search, excitation=exc_1, inhibition=inh_1)

        # Step 2, tracking
        exc_2, inh_2 = self.circuit_2(self.step2_trans(pre_exc_1), excitation=exc_2, inhibition=inh_2, label=bumps)
        ...
        src_template = self.input_proj(src_template)
        dec_rnn = self.rnn_decode_1(self.bn(exc_2))
        proc_rnn = self.rnn_decode_2(self.nl(dec_rnn))   # * torch.sigmoid(self.rnn_gate(dec_rnn))
        hs, _ = self.featurefusion_network(src_template, mask_template, src_search, mask_search, pos_template[-1], pos_search[-1], exc=proc_rnn)
        ...
        if 1:
            ...
            plt.subplot(154);plt.imshow((exc ** 2).squeeze().mean(0).detach().cpu(), vmax=50);plt.title("circuit");plt.axis("off");plt.subplot(155);plt.imshow(ims);plt.axis("off");plt.savefig("gifs/{}_{}.png".format(info['seq_name'], self.count))
            plt.show()
            plt.close(f)
            self.count +=1 
...
class SetCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, boxes, visible, rnn_weight=0.5):
        ...
        if 0:  # 'hgru_boxes' in outputs:
            ...
            tgiou, tiou = box_ops.generalized_box_iou(
                box_ops.box_cxcywh_to_xyxy(all_cb),  # circuit_boxes.flatten(0, 1)),
...
def transt_loss(settings):
    ...
    weight_dict['loss_giou_circuit'] = 0
    losses = ['labels', 'boxes']
