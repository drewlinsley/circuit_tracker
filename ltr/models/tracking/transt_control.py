...
class TransT(nn.Module):
    ...
    def __init__(self, backbone, featurefusion_network, num_classes, input_dim=32, rnn_dims=32, timesteps=32):
    ...
    def forward(self, search, template, samp_idx, labels, settings, boxes):
        ...
        hs = self.featurefusion_network(src_template, mask_template, src_search, mask_search, pos_template[-1], pos_search)  # , exc=proc_rnn[:, :, -1])  # .sigmoid())

        # Concat exc to hs too
        # hs = torch.cat([hs, self.rnn_embed(exc)], -1)
        outputs_class = self.class_embed(hs)
        outputs_coord = self.bbox_embed(hs).sigmoid()
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1], 'hgru_boxes': src_search}
        return out
    ...
    def track(self, search, bumps, info):
        if 0:
            from matplotlib import pyplot as plt
            f = plt.figure()
            plt.imshow(search.cpu().squeeze().permute(1, 2, 0));plt.show()
            plt.close(f)
...
def transt_loss(settings):
    ...
    weight_dict['loss_giou_circuit'] = 0  # Measure this but dont optimize for it
    losses = ['labels', 'boxes']
