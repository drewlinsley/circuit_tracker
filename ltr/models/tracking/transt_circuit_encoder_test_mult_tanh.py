...
class TransT(nn.Module):
    ...
    def __init__(self, backbone, featurefusion_network, num_classes, input_dim=32, rnn_dims=32, timesteps=8):
        ...
        self.circuit_gate = nn.Conv2d(self.rnn_dims ** 2, 1, kernel_size=3, padding=3 // 2)
    ...
    def track(self, search, bumps, info):
        ...
        if td_inh_1 is not None:
            cost_vol = torch.einsum('bik,bjk->bijk', res_hs.view(pre_exc_1.shape[0], pre_exc_1.shape[1], -1), pre_exc_1.view(pre_exc_1.shape[0], pre_exc_1.shape[1], -1)).view(pre_exc_1.shape[0], pre_exc_1.shape[1] ** 2, pre_exc_1.shape[2], pre_exc_1.shape[3])
            rnn_gate = self.circuit_gate(cost_vol).sigmoid()
            src_input = self.input_proj(src_search) + prev_hs * rnn_gate
        else:
            rnn_gate = 0.
            src_input = self.input_proj(src_search)
        hs = self.featurefusion_network(src_template, mask_template, src_input, mask_search, pos_template[0], pos_search[0])  # pos_template[-1] pos_search[-1]

        # Step 3, TD-FB incorporating hs
        res_hs = hs.squeeze().view(1, self.height, self.height, self.hidden_dim).permute(0, 3, 1, 2)
        ...
        if 0:  # td_inh_1 is not None:
            from matplotlib import pyplot as plt
            plt.subplot(151);plt.imshow(ims.squeeze().permute(1, 2, 0).cpu());plt.subplot(152);plt.imshow((self.input_proj(src_search).squeeze()).mean(0).detach().cpu());plt.subplot(153);plt.imshow((rnn_gate).squeeze().detach().cpu())
            plt.subplot(154);plt.imshow((((self.input_proj(src_search) + prev_hs * rnn_gate)).squeeze().mean(0).detach().cpu()));plt.imshow((res_hs).squeeze().mean(0).detach().cpu());plt.show()
            plt.show()
...
class SetCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, boxes, visible):
        ...
        # Finally, look at correlation between rnn_gate and losses + minimize
        idxs = torch.unique(idx[0])
        sum_losses = []
        gates = []
        for i in idxs:
            sum_losses.append(loss_giou[idx[0] == i].sum() / (idx[0] == i).sum())
            if (idx[0] == i).sum():
                gates.append(outputs['rnn_gate'][i])
        vx = torch.stack(sum_losses)
        vx = vx.detach()
        vy = torch.stack(gates)
        if len(vx) and len(vy):
            vx = vx - torch.mean(vx)
            vy = vy - torch.mean(vy)
            cost = torch.sum(vx * vy) / (torch.sqrt(torch.sum(vx ** 2)) * torch.sqrt(torch.sum(vy ** 2)))
            losses['predicted_error'] = 1 - cost
        else:
            losses['predicted_error'] = 0
        return losses
...
def transt_loss(settings):
    ...
    weight_dict['predicted_error'] = 0.5
    losses = ['labels', 'boxes']
