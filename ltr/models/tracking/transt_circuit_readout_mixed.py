...
from ltr.models.neck.circuit_featurefusion_network import build_featurefusion_network
...
class TransT(nn.Module):
    ...
    def __init__(self, backbone, featurefusion_network, num_classes, input_dim=32):
        ...
        self.rnn_proj = nn.Conv2d(backbone.num_channels, hidden_dim // 4, kernel_size=1)
        self.backbone = backbone
        self.reset_hidden = False
        self.circuit = ConvGRUCell(input_dim=hidden_dim, hidden_dim=hidden_dim, kernel_size=5, padding_mode='zeros', batchnorm=True, use_attention=True, timesteps=16)
    ...
    def forward(self, search, template, samp_idx, labels, settings):
        ...
        hs, exc = self.featurefusion_network(self.input_proj(src_template), mask_template, src_search, mask_search, pos_template[-1], pos_search, exc=exc)
