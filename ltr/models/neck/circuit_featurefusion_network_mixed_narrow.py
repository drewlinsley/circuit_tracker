...
class Decoder(nn.Module):
    ...
    def forward(self, tgt, memory, exc,
        ...
        for layer in self.layers:
            output = layer(output, memory, exc=exc, tgt_mask=tgt_mask,
...
class DecoderCFALayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu"):
        ...
        self.mix_q1 = nn.Linear(d_model * 1, d_model)
    ...
    def forward_post(self, tgt, memory, exc,
        ...
        import pdb;pdb.set_trace()
        tgt = self.mix_q3(torch.cat([tgt, self.mix_norm1(self.mix_q2(F.relu(self.mix_q1(exc))))], -1))
        tgt = self.mix_norm2(self.mix_q4(F.relu(tgt)))
        q = self.with_pos_embed(tgt, pos_dec)
        # Or multiply q
...
class FeatureFusionLayer(nn.Module):
    ...
    def forward_post(self, src1, src2,
        ...
        # Alternatively, apply RNN to Q here
